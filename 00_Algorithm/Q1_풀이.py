...
def replaceData(numData): # numData	2차원 정수 배열
    ...
    ###########   여기부터 코딩 (1) ---------------->
    retData = numData[:] #numdata.copy()
    for i in range(5): #range(ARRAY_LENGTH)
        for k in range(5): #range(ARRAY_LENGTH)
            if retData[i][k] < 0:
                retData[i][k]=0
            if retData[i][k] > 100:
                retData[i][k] %= 100
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    for x in range(ARRAY_LENGTH-1):
        for y in range(ARRAY_LENGTH-1):
            sumArray = numData[x][y] + numData[x][y+1] + numData[x+1][y] + numData[x+1][y+1]
            if sumArray > maxSum:
                maxSum = sumArray
